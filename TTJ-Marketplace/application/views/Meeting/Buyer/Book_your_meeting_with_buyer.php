      ...
      <?php $i='1'; foreach($Seller_get  as $row):?> <!-- Buyer Data -->
        ...
        <div class="item-<?= $i ?> m-padding  position-relative">
          <div class="book-buyer-logo  position-relative ">
            ...
            <button class="overlay-button customButton" onclick="customButtonClick(this, '<?= $row->SellerID ?>', '<?= $this->session->userdata('user_id') ?>', '<?= $EventID->EventID ?>','<?= $row->CompanyName ?>')">Button</button>
         </div>
        <?php endif;?>
        <?php endforeach;?>
        <?php $i++; endforeach; ?>
